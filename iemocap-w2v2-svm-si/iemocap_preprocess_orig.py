# IEMOCAP acoustic feature extraction for original data (without augmentation)
# usage: python3 iemocap_preprocess_orig.py /path/to/IEMOCAP_full_release

# IEMOCAP extraction for orig data
import os
...
def main(data_dir):
    ...
    paths.sort()
    out_dir = os.path.join(data_dir, 'meta_data_aug')
    os.makedirs(out_dir, exist_ok=True)
    # for i, path in enumerate(paths):
    os.makedirs(f"{out_dir}", exist_ok=True)
    preprocess(data_dir, paths[:4], path_join(f"{out_dir}", 'train_meta_data.json'))
    preprocess(data_dir, [paths[4]], path_join(f"{out_dir}", 'test_meta_data.json'))
